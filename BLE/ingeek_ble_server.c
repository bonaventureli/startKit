		...
		OperateID[0] |= (uint16_t)Data[3];
			if(OperateID[0] == 0x0101){
				...
				if(((sec_status > 0)&&(sec_status < 255))||(sec_status == 0)){
					...
					//BspDataFlashRead(0xFF200000 + 400,(uint32_t*)gmemorySendata,50);
					memorySendINFOData = (T_INTOTOBLE*)gmemorySendata;
					
					//ret = ingeek_push_info(memorySendINFOData->dataload, memorySendINFOData->Datelength);//0
			...
			switch(uuidID[0]){
				...
				//memcpy(gmemorySendata,INFOData,(INFOData->Datelength)+9);
				//BspDataFlashWrite(0xFF200000 + 400,(uint32_t*)gmemorySendata,50);
				...
				if(ret == SUCCESS){
					...
					if(ingeek_get_sec_status() == READ_INFO){
						...
						if((outlen == 29) ){
						//memcpy(memorySendINFOData,SendINFOData,(SendINFOData->Datelength)+9);
						}
						else{
							return 9;
						}
